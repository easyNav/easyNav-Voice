...
#do not refactor this method, the message is different from the find command
...
def handle(text, mic, profile, dispatcherClient):
    ...
    mic.say("Stopping Previous Navigation")
    mic.say("Please choose your new end point")

    cancel, endLocation, index = getInput(mic);

    if cancel:
        mic.say("Canceling operation......")
        mic.say("Resuming Previous Navigation")
    else:
        strToSay =  endLocation + " found"
        mic.say(strToSay)
        time.sleep(0.2)
        mic.say("Routing to new location....Please wait")
        SUID = getSUID(index)
...
#called by jasper client
def isValid(text):
    return bool(re.search(r'\bchange\b', text, re.IGNORECASE))
